...
class RangeUtils {
	...
	static toRange(object, document = window.document) {
		...
		if (!startContainer.singleNodeValue) {
			// console.log("Highlighter Error: Can Not Match XPath ==> " + object.startContainerPath)
			return null
		}
	...
	static path(node) {
		...
		for (;
			...
			if (node.nodeType === Node.ELEMENT_NODE && node.id.length > 0) {
				// 适配类似于 "s1.pl.1.1" 这种包含与类选择器的语法冲突的点号（.）的元素ID
				let id = node.id.replace(/\./g, '\\.')
				...
				if (node.ownerDocument.querySelectorAll(`#${id}`).length === 1) {
